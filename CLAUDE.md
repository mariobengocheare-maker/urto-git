...
- `Create URTO Desktop Icon.vbs` — one-time setup Mario double-clicks to create the "URTO" Desktop shortcut pointing at `launch_desktop.pyw`. Resolves its own folder via `WScript.ScriptFullName` (not the shell's current directory, which isn't reliable for a double-clicked script), so it works regardless of where the project folder lives.
